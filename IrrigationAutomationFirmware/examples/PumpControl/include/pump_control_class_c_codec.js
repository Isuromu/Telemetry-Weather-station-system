...
function decodeUplink(input) {
  if (input.fPort !== 51 || !input.bytes ||
      !((input.bytes.length === 17 && input.bytes[0] === 1) ||
        (input.bytes.length === 22 && input.bytes[0] === 2))) {
    return { errors: ["Expected PumpControl status: v1/17 bytes or v2/22 bytes on FPort 51"] };
  }
  var b = input.bytes;
  var f = b[1];
  var results = {
    0: "none", 1: "accepted", 2: "failed", 3: "duplicate",
    4: "invalid", 5: "storage_error", 6: "in_progress"
  };
  ...
  if (b[0] === 2) {
    var joinError = readU16(b, 17);
    if (joinError >= 0x8000) joinError -= 0x10000;
    data.previous_join_error_code = joinError;
    data.previous_join_error = joinError === -1116
      ? "No OTAA JoinAccept received in RX1/RX2"
      : joinError === 0 ? "None" : "RadioLib error " + joinError;
    data.join_attempt_count = b[19];
    data.previous_join_retry_seconds = readU16(b, 20);
  }
  return { data: data };
}
...
function encodeDownlink(input) {
  ...
  if (d.command === "refresh") {
    return { fPort: 50, bytes: [1, 5] };
  }
