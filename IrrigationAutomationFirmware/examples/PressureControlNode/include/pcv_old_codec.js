...
// Uplink FPort 31: 28-byte status. Downlink FPort 30: 10-byte command.
...
function decodeUplink(input) {
  ...
  if (!input.bytes || input.bytes.length !== 28)
    return { errors: ["PCV protocol v1 status must contain 28 bytes"] };
  if (input.bytes[0] !== 1)
...
function encodeDownlink(input) {
  ...
  if (flags === 0)
    return { errors: ["set the PCV action, report interval, or both"] };
  ...
  var bytes = [1, flags, action, 0, 0, 0, 0, 0, 0, 0];
