...
- Добавлено чтение REG0113-REG0118: net, positive и negative accumulated
  volume непосредственно в кубических метрах.
- Сброс накопителей прибора намеренно не отправляется: руководство описывает
  интерактивный M37, но не задаёт прямой Modbus-регистр сброса.

## v0.9.1

- Порядок REAL4 подтверждён на приборе как `LOW_WORD_FIRST`: сырые
  REG0221-REG0222 `00 00 42 64` декодируются как настроенные `57.0 mm`.
- Профиль TUF теперь показывает одно подтверждённое значение расхода,
  скорости, M91 и внутреннего диаметра без строк `candidate`.
