...
def low_word_first_float(value: float) -> bytes:
    ieee = struct.pack(">f", value)
    return ieee[2:4] + ieee[0:2]
...
class Tuf2000mProfileTests(unittest.TestCase):
    ...
    def test_version_and_serial_settings(self) -> None:
        self.assertEqual(app.APP_VERSION, "0.9.2")
        self.assertEqual(self.profile["baud"], 9600)
        self.assertEqual(self.profile["serial_config"], "8N1")
        self.assertEqual(
            self.profile["status"], "supported_low_word_first_hardware_verified"
        )
    ...
    def test_low_word_first_measurements(self) -> None:
        data = (
            low_word_first_float(12.5)
            + low_word_first_float(0.0)
            + low_word_first_float(1.25)
        )
        ...
        self.assertEqual(parsed["flow_rate"].value, 12.5)
        self.assertEqual(parsed["velocity"].value, 1.25)

    def test_low_word_first_inner_diameter_matches_hardware_evidence(self) -> None:
        frame = fc03_response(low_word_first_float(57.0))
        block = self.profile["read"]["blocks"][5]
        parsed = {value.name: value for value in app.ProfileParser.parse_spec(block, frame)}
        self.assertEqual(parsed["inner_diameter"].value, 57.0)
        self.assertEqual(parsed["inner_diameter"].status, "OK")
    ...
    def test_signal_quality_and_raw_strengths(self) -> None:
        ...
        frame = fc03_response(data)
        block = self.profile["read"]["blocks"][3]
        parsed = {value.name: value for value in app.ProfileParser.parse_spec(block, frame)}
    ...
    def test_flow_accumulators_are_cubic_metres(self) -> None:
        frame = fc03_response(
            low_word_first_float(10.0)
            + low_word_first_float(11.0)
            + low_word_first_float(1.0)
        )
        block = self.profile["read"]["blocks"][1]
        parsed = {value.name: value for value in app.ProfileParser.parse_spec(block, frame)}
        self.assertEqual(parsed["net_total_volume"].value, 10.0)
        self.assertEqual(parsed["positive_total_volume"].value, 11.0)
        self.assertEqual(parsed["negative_total_volume"].value, 1.0)
    ...
    def test_multi_block_read_keeps_every_tx_rx_and_parsed_value(self) -> None:
        responses = [
            ...
            fc03_response(low_word_first_float(100.0)),
            fc03_response(low_word_first_float(57.0)),
        ...
        self.assertEqual(record["successful_blocks"], 7)
        self.assertEqual(record["total_blocks"], 7)
        self.assertEqual(len(record["transactions"]), 7)
        self.assertEqual(len(record["values"]), 13)
        self.assertEqual(
