...
- Профиль `TUF-2000M + TS-2 Ultrasonic Flow Meter`:
  - единая многошаговая проверка расхода, скорости, M08, M90, M91,
    накопленного объёма, внутреннего диаметра и M46;
...
5. В `Raw transaction / request status` отображаются полные TX/RX кадры с CRC
   для всех семи запросов.

REG0113-REG0118 показывают net, positive и negative accumulated volume в
кубических метрах. Профиль остаётся read-only и не отправляет сброс M37.
