# JXSZ-1001-SS Water Suspended Solids Sensor (RS485)

Source: `JXSZ-1001-Water Suspended.pdf`

## 1) Overview
- **Purpose / measurements:** Suspended solids concentration (and temperature).
...
**Notes:** Manual includes register table: temp at 0x0001 and suspended matter at 0x0002. Scale text and example slightly conflict; treat example as authoritative and confirm on hardware.
...
| 0x0002 | Suspended solids | 0x03 | u16 | BE | UNCERTAIN (/10 vs /100) | mg/L | sensor-dependent | Table text says 0.01 mg/L but example maps 189 → 18.9, implying /10 |
...
### Read suspended solids (1 register)

**Request (WITHOUT CRC):** `01 03 00 02 00 01`
...
ss_mg_l = u16(ssRaw)/10.0  (per example; confirm if /100 is needed from your unit)

**Worked example (from manual if available):**

Example: 0x00BD=189 → 18.9 (units per manual context).
...
|---|---:|---:|---|---|
| Suspended solids | 0 | 20000 | mg/L | raw u16? (per manual: range 0–20000 mg/L) |
| Water temperature (if provided) | 0 | 50 | °C | manual lists operating temp 0–50°C |
