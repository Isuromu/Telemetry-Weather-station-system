...
| `JXEC_T_Series_Conductivity_Probe_Water_EC_TDS_Temp_RS485.md` | Water temperature: -10..80 C; Conductivity: 0..configured max uS/cm | Conductivity is raw u32 from registers 0x0002/0x0003; K=1 example uses raw / 100.0, but scale and max must match the probe/controller range |
...
| `Water_EC_Sensor_RS485_project_item_maps_to_conductivity_probe_doc.md` | Water temperature: -10..80 C; Conductivity: 0..configured max uS/cm | Same protocol as JXEC-T controller + probe; confirm the hardware is RS485 transmitter/controller assembly, not bare probe only |
