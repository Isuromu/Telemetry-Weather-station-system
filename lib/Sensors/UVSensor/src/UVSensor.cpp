...
UVSensor::UVSensor(RS485Bus &bus, uint8_t address)
    : _bus(bus), _addr(address) {}

void UVSensor::setAddress(uint8_t address) { _addr = address; }
uint8_t UVSensor::address() const { return _addr; }

bool UVSensor::readAll(UVSensor_Data &out) {
  // --- Read 1: UV at reg 0x0008, qty 1 → 7 bytes ---
  {
    uint8_t tx[8] = {_addr, 0x03, 0x00, 0x08, 0x00, 0x01, 0, 0};
    ...
    uint16_t uvRaw = ((uint16_t)rx[3] << 8) | rx[4];
    float uv = uvRaw / 10.0f;

    // Hard bounds: 0–150 W/m²
    if (uv < 0.0f || uv > 150.0f)
      return false;
    out.uv_w_m2 = uv;
  }

  // --- Read 2: T+H at regs 0x0000–0x0001, qty 2 → 9 bytes ---
  {
...
bool UVSensor::changeAddress(uint8_t newAddress) {
