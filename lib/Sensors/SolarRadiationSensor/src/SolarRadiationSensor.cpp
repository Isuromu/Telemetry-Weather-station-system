#include "SolarRadiationSensor.h"

SolarRadiationSensor::SolarRadiationSensor(RS485Bus &bus, uint8_t address)
    : _bus(bus), _addr(address) {}

void SolarRadiationSensor::setAddress(uint8_t address) { _addr = address; }
uint8_t SolarRadiationSensor::address() const { return _addr; }

bool SolarRadiationSensor::readAll(SolarRadiationSensor_Data &out) {
  ...
  // Hard bounds: 0–1500 W/m²
  if (raw > 1500)
    return false;

  out.radiation_w_m2 = raw;
  return true;
}

bool SolarRadiationSensor::changeAddress(uint8_t newAddress) {
