                     ...
                     uint32_t config)
{
  ...
  #else
    (void)rxPin; (void)txPin; (void)config;
    serial.begin(baud);
  #endif
...
void RS485Bus::setLogger(PrintController* logger){
  _log = logger;
}
                           ...
                           uint32_t interByteTimeoutMs)
{
  ...
  _dir.setTX(true);
  if (_preTxDelayUs) delayMicroseconds(_preTxDelayUs);
  ...
  if (_postTxDelayUs) delayMicroseconds(_postTxDelayUs);
  _dir.setTX(false);
...
bool RS485Bus::extractFixedFrameByPrefix(const uint8_t* prefix, size_t prefixLen,
                                         size_t expectedFrameLen,
                                         uint8_t* outFrame, size_t outFrameMax,
                                         size_t* outIndex) const
{
  if (!prefix || prefixLen == 0 || expectedFrameLen == 0) return false;
  if (!outFrame || outFrameMax < expectedFrameLen) return false;
  if (_rxLen < expectedFrameLen) return false;

  // Scan raw buffer for prefix match
  for (size_t i = 0; i + expectedFrameLen <= _rxLen; i++) {
    bool ok = true;
    for (size_t k = 0; k < prefixLen; k++) {
      if (_rxBuf[i + k] != prefix[k]) { ok = false; break; }
    }
    if (ok) {
...
bool RS485Bus::transferAndExtractFixedFrame(const uint8_t* tx, size_t txLen,
                                           ...
                                           size_t* outIndex)
{
  ...
  if (!gotAny) {
    if (_log && _log->isEnabled() && _log->verbosity() >= PrintController::SUMMARY) {
  ...
  size_t idx = 0;
  bool ok = extractFixedFrameByPrefix(prefix, prefixLen, expectedFrameLen, outFrame, outFrameMax, &idx);
  ...
  if (_log && _log->isEnabled()) {
    ...
    if (_log->verbosity() >= PrintController::TRACE) {
      if (ok) {
        ...
        _log->println((unsigned long)idx);
      }
    }
  }
...
// ---- logging internals ----
void RS485Bus::logSummaryTransfer(bool gotAny, size_t txLen, size_t rxLen) const {
  if (!_log || !_log->verbosity() >= PrintController::SUMMARY) return;
