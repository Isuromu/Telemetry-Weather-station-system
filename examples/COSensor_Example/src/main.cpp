...
static COSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=============================================="));
  printer.println(F("  FIRMWARE: CO Carbon Monoxide Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
    ...
    lastPoll = millis();
    COSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling CO Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("CO: "));
      printer.print(data.co_ppm, 1);
