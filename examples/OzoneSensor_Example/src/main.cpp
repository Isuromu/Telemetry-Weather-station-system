...
static OzoneSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("================================"));
  printer.println(F("  FIRMWARE: Ozone Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    OzoneSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling Ozone Sensor..."));
    if (sensor.readAll(data)) {
      printer.logln(PrintController::TRACE,
                    F("\n===== OZONE SENSOR DATA ====="));
      printer.log(PrintController::TRACE, F("O3          : "));
      printer.print(data.ozone_ppm, 2);
