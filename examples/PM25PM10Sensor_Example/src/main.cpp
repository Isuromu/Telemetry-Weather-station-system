...
static PM25PM10Sensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("======================================"));
  printer.println(F("  FIRMWARE: PM2.5/PM10 Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    PM25PM10Sensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling PM2.5/PM10 Sensor..."));
