...
static NH3Sensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("======================================"));
  printer.println(F("  FIRMWARE: NH3 Ammonia Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
    ...
    lastPoll = millis();
    NH3Sensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling NH3 Sensor..."));
    if (sensor.readAll(data)) {
      printer.logln(PrintController::TRACE, F("\n===== NH3 SENSOR DATA ====="));
      printer.log(PrintController::TRACE, F("NH3         : "));
      printer.print(data.nh3_ppm, 1);
