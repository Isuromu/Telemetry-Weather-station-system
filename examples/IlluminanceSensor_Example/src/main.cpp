...
static IlluminanceSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("========================================="));
  printer.println(F("  FIRMWARE: Illuminance Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    IlluminanceSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling Illuminance Sensor..."));
