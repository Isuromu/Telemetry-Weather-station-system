...
static SmallLeafTemperatureHumidity leaf(
    rs485,
    SENSOR_ID,
    SENSOR_ADDRESS,
    SENSOR_DEBUG,
...
static void printBanner() {
  ...
  printer.println(F("============================================================"), true);
  printer.println(F(" Small Leaf Temperature/Humidity Diagnostic Example"), true);
  printer.println(F("============================================================"), true);
  ...
  printer.println(F("- Loop reads small leaf temperature and humidity"), true);
  printer.println(F("- Scaling: temperature raw / 100, humidity raw / 100"), true);
  printer.println(F(""), true);
}
...
static void printReadResult(bool ok) {
  ...
  printer.print(F("Leaf Temperature raw: "), true);
  printer.print((int)leaf.leaf_temperature_raw, true, " | ", DEC);
  printer.print(F("Leaf Temperature: "), true);
  printer.print(leaf.leaf_temperature, true, " C | ", 2);

  printer.print(F("Leaf Humidity raw: "), true);
  printer.print((unsigned int)leaf.leaf_humidity_raw, true, " | ", DEC);
  printer.print(F("Leaf Humidity: "), true);
  printer.print(leaf.leaf_humidity, true, " %RH", 2);
  printer.println("", true);
...
void setup() {
  ...
  if (ADDRESS_CHANGE_AT_BOOT) {
    runAddressChangeAtBoot(
        leaf,
        printer,
        ADDRESS_CHANGE_NEW_ADDRESS,
        F("Only the target small leaf sensor should be connected; address register is 0x0030."));
  }
