...
static EvaporationSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=========================================="));
  printer.println(F("  FIRMWARE: Evaporation Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    EvaporationSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling Evaporation Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("Evaporation (raw): "));
      printer.println(data.evaporation_raw);
      // Note: units (mm or g) to be confirmed on hardware — see driver TODO
      // comment.
    } else {
