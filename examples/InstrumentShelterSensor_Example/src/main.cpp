...
static InstrumentShelterSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=============================================="));
  printer.println(F("  FIRMWARE: Instrument Shelter Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    InstrumentShelterSensor_Data data{};
    printer.logln(PrintController::TRACE,
                  F("Polling Instrument Shelter Sensor..."));
    if (sensor.readAll(data)) {
      printer.logln(PrintController::TRACE,
                    F("\n===== INSTRUMENT SHELTER DATA ====="));
      ...
      printer.log(PrintController::TRACE, F("Illuminance : "));
      printer.print(data.lux);
      printer.println(F(" Lux"));
      printer.logln(PrintController::TRACE,
                    F("===================================\n"));
    } else {
