...
static UVSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=============================="));
  printer.println(F("  FIRMWARE: UV Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
    ...
    lastPoll = millis();
    UVSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling UV Sensor..."));
    if (sensor.readAll(data)) {
      printer.logln(PrintController::TRACE, F("\n===== UV SENSOR DATA ====="));
      printer.log(PrintController::TRACE, F("UV Intensity : "));
      printer.print(data.uv_w_m2, 1);
