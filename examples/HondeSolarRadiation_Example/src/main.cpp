...
static HondeSolarRadiation solar(
    rs485,
    SENSOR_ID,
    SENSOR_ADDRESS,
    SENSOR_DEBUG,
...
static void printBanner() {
  ...
  printer.println(F("============================================================"), true);
  printer.println(F(" Honde Solar Radiation Diagnostic Example"), true);
  printer.println(F("============================================================"), true);
  ...
  printer.println(F("- Loop reads solar radiation in W/m2"), true);
  printer.println(F(""), true);

  printer.println(F("[INSTALL] Wiring: Red=V+, Black=GND, Yellow=RS485-A, Green=RS485-B"), true);
  printer.println(F("[INSTALL] Install with clear sky view and avoid shadows."), true);
  printer.println(F("[INSTALL] Level the sensor with the bubble level; terminal/cable side faces north."), true);
  printer.println(F("[INSTALL] Keep the glass dome clean with a soft cloth and water or alcohol."), true);
  printer.println(F(""), true);
}

static void printReadResult(bool ok) {
  printer.print(F("[APP] Sensor ID: "), true);
  printer.print(solar.getSensorId(), true, " | ");
  printer.print(F("Address: 0x"), true);
  printer.print((unsigned int)solar.getAddress(), true, " | ", HEX);
  ...
  printer.print(F("Solar radiation: "), true);
  printer.print(solar.solar_radiation, true, " W/m2", 0);
  printer.println("", true);

  if (!ok) {
    printer.print(F("[APP] Error count: "), true);
    printer.println((unsigned int)solar.getConsecutiveErrors(), true);
  }
}
...
void setup() {
  ...
  // the button before calling solar.changeAddress().
  if (ADDRESS_CHANGE_AT_BOOT) {
    runAddressChangeAtBoot(solar,
                           printer,
                           ADDRESS_CHANGE_NEW_ADDRESS,
                           F("Only the target sensor should be connected; Honde solar writes address register 0x0501 with function 0x10."));
  }

  if (DO_SCAN) {
    printer.println(F("[APP] Scan mode enabled. Searching sensor address..."), true);
    const uint8_t found = solar.scanForAddress(1, 247, 150, SENSOR_DEFAULT_AFTER_REQ_MS);
    if (found != 0) {
