...
static PARSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=============================="));
  printer.println(F("  FIRMWARE: PAR Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
    ...
    lastPoll = millis();
    PARSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling PAR Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("PAR: "));
      printer.print(data.par_w_m2);
