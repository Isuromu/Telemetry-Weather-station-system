...
static void printBanner() {
  ...
  printer.println(F("============================================================"), true);
  printer.println(F(" Rika Rain Gauge Diagnostic Example"), true);
  printer.println(F("============================================================"), true);
  ...
  printer.println(F("[APP] Pulse factor: 0.2 mm/pulse"), true);
...
void setup() {
  ...
  if (RAIN_COUNTING_MODE == RAIN_GAUGE_COUNTING_MODE_RS485_SENSOR && ADDRESS_CHANGE_AT_BOOT) {
    runAddressChangeAtBoot(rain, printer, ADDRESS_CHANGE_NEW_ADDRESS, F("Only the target Rika rain gauge should be on the RS485 bus."));
  }
