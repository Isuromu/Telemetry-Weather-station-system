...
static AtmosphericPressureSensor sensor(bus, targetAddress);
...
void scanAddresses() {
  ...
  for (uint8_t addr = 1; addr <= 247; addr++) {
    ...
    req[6] = crc & 0xFF;
    req[7] = crc >> 8;
    if (bus.transferRaw(req, 8, SCAN_TIMEOUT_MS + 150)) {
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("================================================="));
  printer.println(F("  FIRMWARE: Atmospheric Pressure Sensor Example"));
  printer.println(F("  Program starts here (reset point)."));
  printer.println(F("  NOTE: Pressure register offset is UNCERTAIN."));
  printer.println(F("  See driver TODO comment & verify on hardware."));
  printer.println(F("================================================="));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    AtmosphericPressureSensor_Data data{};
    printer.logln(PrintController::TRACE,
                  F("Polling Atmospheric Pressure Sensor..."));
    if (sensor.readAll(data)) {
      printer.logln(PrintController::TRACE,
                    F("\n===== ATMOSPHERIC PRESSURE SENSOR ====="));
      printer.log(PrintController::TRACE, F("Pressure    : "));
      printer.print(data.pressure_mbar, 2);
      printer.println(F(" mbar"));
