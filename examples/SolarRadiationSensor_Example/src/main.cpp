...
static SolarRadiationSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=========================================="));
  printer.println(F("  FIRMWARE: Solar Radiation Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    SolarRadiationSensor_Data data{};
    printer.logln(PrintController::TRACE,
                  F("Polling Solar Radiation Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("Solar Radiation: "));
      printer.print(data.radiation_w_m2);
      printer.println(F(" W/m2"));
    } else {
