...
static RainSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("================================"));
  printer.println(F("  FIRMWARE: Rain Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    RainSensor_Data data{};
    printer.logln(PrintController::TRACE, F("Polling Rain Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("Rainfall: "));
      printer.print(data.rainfall_mm, 1);
      printer.println(F(" mm"));
    } else {
