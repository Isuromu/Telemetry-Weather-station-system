...
static WindDirectionSensor sensor(bus, targetAddress);
...
void setup() {
  ...
  printer.println(F(""));
  printer.println(F("=========================================="));
  printer.println(F("  FIRMWARE: Wind Direction Sensor Example"));
...
void loop() {
  ...
  if (millis() - lastPoll >= static_cast<uint32_t>(POLL_INTERVAL_MS) ||
      lastPoll == 0) {
    lastPoll = millis();
    WindDirectionSensor_Data data{};
    printer.logln(PrintController::TRACE,
                  F("Polling Wind Direction Sensor..."));
    if (sensor.readAll(data)) {
      printer.log(PrintController::TRACE, F("Wind Direction: "));
      printer.print(data.direction_deg);
      printer.println(F(" deg"));
    } else {
